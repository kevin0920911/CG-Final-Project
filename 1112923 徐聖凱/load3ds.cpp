...
char Load3DS(obj_type_ptr p_object, char* p_filename)
{
	...
	if (strcmp(p_filename,"\0") == 0) return(0);

	if ((l_file = fopen(p_filename, "rb")) == NULL)
	{
		MessageBox(NULL, (LPCWSTR)"3DS file not found", (LPCWSTR)"Spacesim", MB_OK | MB_ICONERROR);
		return (0);
	}
	...
	while (ftell(l_file) < filelength(fileno(l_file))) //Loop to scan the whole file 
	{
		...
		case 0x4110:
			...
			{
				MessageBox(NULL, (LPCWSTR)"Number of vertices too high", (LPCWSTR)"Spacesim", MB_OK | MB_ICONERROR);
				return (0);
			}
			for (i = 0; i < l_qty; i++)
			{
		...
		case 0x4120:
			...
			{
				MessageBox(NULL, (LPCWSTR)"Number of polygons too high", (LPCWSTR)"Spacesim", MB_OK | MB_ICONERROR);
				return (0);
			}
			for (i = 0; i < l_qty; i++)
			{
